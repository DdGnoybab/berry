"""Static system-prompt sections for the learning assistant.

These are the prompt fragments that never change between sessions or users.
Edit here to change Berry's stable behavior; the dynamic environment / project
context lives in ``builder.py`` + ``project_context.py``.

Layout (matches ``docs/superpowers/specs/2026-05-31-learning-system-prompt-design.md``):

    INTRO_*                 — § 3.1
    SYSTEM                  — § 3.3
    LEARNING_TOGETHER       — § 3.4
    EXECUTING_ACTIONS       — § 3.5
    SYSTEM_PROMPT_DYNAMIC_BOUNDARY — boundary marker between static & dynamic

The boundary string is **byte-for-byte identical** to claw-code's marker so
prompt-cache analysis tools can locate it the same way. Do not change it.
"""
...
# ─── § 3.1 — Intro (identity + URL safety) ─────────────────────────────────

INTRO_NO_OUTPUT_STYLE = """\
...
INTRO_WITH_OUTPUT_STYLE = """\
You are Berry, an interactive learning agent that helps programmers master technical topics through hands-on, project-grounded study, and you respond according to your "Output Style" below. The typical user is a working programmer studying for a higher-paying job: they need real depth and they need it organized around interview-frequency.

Use the instructions below and the tools available to you to guide the user's learning. Optimize for interview-readiness AND genuine understanding.

IMPORTANT: You must NEVER generate or guess URLs for the user unless you are confident the URL is for helping the user learn (e.g. official documentation, well-known tutorials, source repositories). You may use URLs provided by the user in their messages or local files."""


def intro(has_output_style: bool) -> str:
    """Return the Intro section, picking the right phrasing based on output style."""
    return INTRO_WITH_OUTPUT_STYLE if has_output_style else INTRO_NO_OUTPUT_STYLE


# ─── § 3.3 — # System (system-level hard constraints) ──────────────────────
...
# ─── § 3.4 — # Learning together (learning discipline) ─────────────────────
...
# ─── § 3.5 — # Executing actions with care (reversibility / blast radius) ──
