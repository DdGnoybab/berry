"""MethodRegistry — Berry process-internal method-name → handler dispatcher.

Transport-agnostic. Channels (web SSE/RPC, CLI, feishu) all dispatch via
``registry.call(name, params, ctx)``; they don't care about how the
handler is implemented.

Lives under ``core/agent`` (not ``gateway/``) because the registry
itself doesn't depend on any specific transport or channel —
only on ``protocol`` schemas. Handler IMPLEMENTATIONS still live in
``gateway/methods/`` (they assemble runtime, db, etc.).

History: this module was at ``berry/gateway/methods/registry.py``
through ADR-0009; the move was made when web became a channel and
needed to dispatch methods without importing ``gateway/`` (which
would violate rule 3a "channels do not depend on gateway"). The
move keeps ``handler implementations`` in gateway and ``dispatcher
infrastructure`` in core, which is the cleaner split.
"""
